...
def turn_off_after(entity_id: str):
    task.sleep(5)
...
def run(entity_id: str, new_state: str):
    ...
    if new_state == 'on':
        task_by_entity[entity_id] = task.create(turn_off_after, entity_id)


@service
def light_timer(entity_id: str, new_state: str):
    run(entity_id, new_state)
